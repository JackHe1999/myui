...
export const show: ContainerOptions['show'] = (props, id, innerStyle) => {
  ...
  (Object.keys(style) as Array<keyof CSSProperties>).forEach((k: keyof CSSProperties) => {
  ...
  if (innerStyle) {
    (Object.keys(innerStyle) as Array<keyof CSSProperties>).forEach((k: keyof CSSProperties) => {
...
export const move: ContainerOptions['move'] = (id, pos) => {
  if (id === currentId && div) {
    ...
    (Object.keys(pos) as Array<keyof typeof pos>).map((key: keyof typeof pos) => (div!.style[key] = transStyle(pos[key])))
  }
}
