...
import _ from '../../locate'
import { exampleClass } from '../../styles'
import Icon from '../../icons/Icon'
import { exampleTree, exampleIds } from '../../data/tree'
import { fCitys } from '../../data/city'
import { fetchSync } from '../../data/user'
import lorem from '../../utils/faker/lorem'
...
export default ${file}`,
  ...
  const transText = text.replace(/('|")myui('|")/g, '"myui/dist/myui.min.js"')
  useEffect(
    () => {
      const r = {
        'package.json': {
...
import 'myui/dist/theme.myui.css'
...
`,
      ...
      imports.forEach(i => {
        if (i.indexOf('.') === -1) {
          if (i === 'myui') {
            if (!r['package.json'].content.dependencies.myui)
              r['package.json'].content.dependencies.myui = process.env.LOG_ENV === 'rc' ? 'next' : 'latest'
