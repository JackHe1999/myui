...
import { Lazyload, Spin, Tabs } from '@mycomponents'
import Icon from '@src/icons/Icon'
import { exampleClass } from '@src/styles'
...
export default function Example({ component, id, name, rawText, title: propsTitle, ...rest }) {
  ...
  const commendReg = /(^|\n|\r)\s*\/\*[\s\S]*?\*\/\s*(?:\r|\n|$)/
  // const tsToJsReg = /:\s*\w+(\[\])?/g;
  // const tsToJsReg = /:\s*((?:[\w.]+(?:<[\w<>,\s]+>)?)(?:\[\])?)/g;
  const tsToJsReg = /:\s*((?:[\w.]+(?:<[\w<>,\s]+>)?)(?:\[\])?)/;
  const text = rawText.replace(commendReg, '').trim()
  const parseText = (text || '').replace(tsToJsReg, '').trim()
